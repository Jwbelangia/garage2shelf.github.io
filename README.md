# YourCar-n-Hand
3d resin prints of your car. Painted or unpainted.

## One-page site

Open `index.html` in a browser (or serve the folder with a static server).

### Included features
- Hero section with a central GIF area (`assets/resin-turntable-placeholder.svg` can be replaced by your GIF)
- Process gallery thumbnails
- Order form with:
  - Email
  - Click-to-upload car angle images (Front, Left, Right, Rear, Diagonal)
  - Resin finish dropdown (Unprimed / Primed / Painted)
  - Delivery address
  - PayPal $20 deposit note + link
- Submission to Google Apps Script endpoint using script ID:
  - `10AlswStNEky-fd_ueqdRlIzEffGZfdveXy8LeQKOlHsQZ7ITUDBEY67b`
- Bottom slow-fade showcase gallery

You can update process and bottom gallery image lists in `script.js`.
